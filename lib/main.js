...
var streamAPI = 'https://api.twitch.tv/kraken/streams/',
    ...
    followingSuff = '/follows/channels',
...
exports.main = function(options, callbacks) {
    ...
    if (options.loadReason == "install" || options.loadReason == "upgrade" || ss.storage.settings.length == 0) {
        setStorageItem('notification', true);
        setStorageItem('notifySound', true);
        setStorageItem('open', 1);
        setStorageItem('notifySoundRing', 'Ring.mp3');
    }
    ...
    function deleteStorageItem(item) {
        delete ss.storage.settins[item];
    }
    ...
    function initializeNotify() {
        ...
        if (username != undefined && username != '') {
            followingCheck(username, function() {
                // Firstly we are initializing a postnotify and then setting it to do that for 5mins(300000 ms) repeatedly.
                postNotify();
    ...
    function followingCheck(username, callback) {
        if (username != undefined) {
            var followUrl = followingPre + encodeURIComponent(username) + followingSuff;
    ...
    panel.port.on('getSignInOut', function() {
        ...
        if (getStorageItem('twitchName') == null) {
    ...
    panel.port.on('getSettings', function(isLogged) {
        if (isLogged) {
            panel.port.emit('settingsResponse', 
                _('logged_as'), 
                signOut, 
                _('refresh_fl_btn'), 
                _('refresh_followed'), 
                _('donate'), 
                _('created_by'),
                getStorageItem('notification'),
                getStorageItem('notifySound'),
                getStorageItem('open'),
                getStorageItem('notifySoundRing'));
        } else {
